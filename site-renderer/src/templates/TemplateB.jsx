...
export default function TemplateB({ site, articles, categories, categoryMap, theme, pageTitle, headerExtra }) {
  ...
  return (
    <div dir={dir} style={{ ...theme, backgroundColor: 'var(--color-bg)', color: 'var(--color-text)' }} className="min-h-screen">
      ...
      {/* Hero */}
      {hero && (
        <div className="relative h-48 md:h-[280px] overflow-hidden" style={{ backgroundColor: 'var(--color-primary)' }}>
          <img
            src={hero.main_image_url || getDefaultImage(siteKeywords, 0)}
            alt=""
        ...
        {spotlight.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            {spotlight.map((article) => (
              <Link key={article.id} to={`/article/${article.id}`} className="group relative overflow-hidden rounded-xl h-56" style={{ backgroundColor: 'var(--color-primary)' }}>
                <img
                  src={article.main_image_url || getDefaultImage(siteKeywords, article.id)}
                  alt=""
              ...
              {rest.map((article) => (
                <Link key={article.id} to={`/article/${article.id}`} className="group flex flex-col rounded-xl overflow-hidden"
                  ...
                  <img
                    src={article.main_image_url || getDefaultImage(siteKeywords, article.id)}
                    alt=""
                    className="w-full aspect-video object-cover group-hover:brightness-95 transition-all"
                  />
